...
const symptomFormSchema = z.object({
  ...
  severity: z.enum(symptomSeverities, { required_error: 'Severity is required.' }),
...
const AddSymptomForm: React.FC<AddSymptomFormProps> = ({ onSuccess }) => {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('forms.symptom.name')}</FormLabel>
              <FormControl>
                <Input placeholder={t('forms.symptom.namePlaceholder')} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('forms.symptom.severity')}</FormLabel>
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl>
                  <SelectTrigger>
                    <SelectValue placeholder={t('forms.symptom.severityPlaceholder')} />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
                  {symptomSeverities.map((level) => (
                    <SelectItem key={level} value={level}>
                      {t(`enums.symptomSeverity.${level}`)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('forms.symptom.notes')}</FormLabel>
              <FormControl>
                <Textarea placeholder={t('forms.symptom.notesPlaceholder')} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
